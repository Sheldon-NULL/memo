...
/*                int HourNow = int.Parse(DateTime.Now.Hour.ToString());
        ...
        {
            timer.Text = "现在的时间是：" + DateTime.Now;
        }
 ...
 //           ofd.InitialDirectory = @"E:\音乐";//设置初始目录
        ...
        {  //未选中行则默认第一行
            ...
            try
            {
                MusicPlayer1.URL = Listpath[MusicBox1.SelectedIndex];//路径
                MusicPlayer1.Ctlcontrols.play();//播放
                lblShow.Text = MusicPlayer1.URL.Remove(0, 6);
        ...
        private void 设置或更换图片PToolStripMenuItem_Click(object sender, EventArgs e)
        {
            OpenFileDialog ofd = new OpenFileDialog();//创建一个文件对话框
